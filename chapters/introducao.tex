...
A superação dessas barreiras ocorreu com o advento de técnicas de Aprendizado Profundo (\textit{Deep Learning}) e, mais recentemente, com a popularização das LLM's\cite{vaswani2017attention}. Fundamentados majoritariamente na arquitetura \textit{Transformer}, esses modelos utilizam mecanismos de atenção para ponderar a relevância de diferentes elementos em uma sequência textual, permitindo a compreensão de dependências de longo prazo e contextos complexos \cite{vaswani2017attention}. Ao serem treinados em \textit{datasets} massivos que abrangem grande parte do conhecimento disponível na internet, os LLM's adquiriram uma capacidade de generalização inédita. Isso permitiu que a tecnologia evoluísse de tarefas simples de classificação para a geração autônoma de texto, raciocínio lógico e adaptação a diferentes domínios de conhecimento sem a necessidade de retreinamento específico \cite{zhao2023survey}.
...
\begin{description}
    \item[QP01] O quão acertivo pode ser as LLM's na resolução de questões em computação?
...
\begin{itemize}
    ...
    \item Coletar e organizar questões objetivas de múltipla escolha do ENADE, aplicadas nos últimos 20 anos, para os cursos da área de Computação;
